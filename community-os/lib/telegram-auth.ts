...
/** Ensures a `community_profiles` row exists for the given internal user id. */
async function ensureProfile(userId: number): Promise<void> {
  ...
  const { error } = await supabase.from('community_profiles').insert({ user_id: userId })
  // Ignore unique-violation races (profile created concurrently).
  if (error && error.code !== '23505') throw error
}

/** Looks up the internal user row for a Telegram user, creating it (and a community profile) if needed. */
