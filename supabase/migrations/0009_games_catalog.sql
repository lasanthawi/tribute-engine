...
--
-- 0007_minigame_hardening.sql added CHECK constraints and a unique index that
-- hardcode game_type IN ('tap','spin'). Since games are now identified by a
-- dynamic slug (any published row in `games`), those constraints are dropped
-- and replaced with generic, slug-agnostic equivalents below.

ALTER TABLE minigame_plays DROP CONSTRAINT IF EXISTS minigame_plays_game_type_check;
ALTER TABLE minigame_plays DROP CONSTRAINT IF EXISTS minigame_plays_score_check;
DROP INDEX IF EXISTS idx_minigame_plays_one_spin_per_day;

CREATE TABLE games (
  id                BIGSERIAL PRIMARY KEY,
  slug              TEXT UNIQUE NOT NULL,
  template          TEXT NOT NULL,                  -- 'tap_catch' | 'spin_wheel' | 'gomoku'
  title             TEXT NOT NULL,
