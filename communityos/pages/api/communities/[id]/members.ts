...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (isDemoMode) {
    if (req.method === 'GET') return res.status(200).json({ members: demoDashboard.members })
    if (req.method === 'POST') return res.status(201).json({ member: demoDashboard.members[0] })
  }
