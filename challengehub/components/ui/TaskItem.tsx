...
function dueDateForDay(challengeStartDate: string, day: number): Date {
...
export default function TaskItem({
  ...
  const due = challengeStartDate ? dueDateForDay(challengeStartDate, task.day) : null
  const missed = !done && !!due && due.getTime() < today.getTime()
    ...
    <div className={`task-item-wrap ${expanded ? 'expanded' : ''}`}>
      ...
      {expanded && (
        <div className="checkin-form card-in">
          ...
          <button className="btn-ghost" onClick={toggleExpand}>
            Cancel
          </button>
        </div>
      )}
